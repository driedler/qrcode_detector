...
CodeFinder::CodeFinder(Mat image, bool hasCode)
	...
	if (image.cols > 2000 || image.rows > 2000) {
		DEBUG("Resizing Image, because it is too large: %dx%d\n", image.rows, image.cols);
...
Mat CodeFinder::find() {
	...
	DEBUG("Converting image to binary image...\n");
	Mat grayscaleImage;
	...
	do {
		...
		DEBUG("	Finding all contours...\n");
		findAllContours();

		DEBUG("	Finding all finder pattern candidates...\n");
		findPatternContours();
		DEBUG("	Number of detected patterns: %d\n", (int)allFinderPatterns.size());
		...
		if (allFinderPatterns.size() < 3) {
			if (thresholdMethod < maxThresholdMethod) {
				DEBUG("	Try different ThresholdMethod\n\n");
				continue;
			}
			DEBUG("Could not find any valid Patterns.\n");
			return drawNotFound();
		}

		DEBUG("	Finding all edge lines for finder patterns...\n");;
		findPatternLines();
		DEBUG("	Number of detected valid patterns: %d\n", (int)validFinderPatterns.size());
	...
	DEBUG("Successfully located FinderPatterns.\n");

	DEBUG("Iterating all combinations of detected finder patterns...\n");
	bool isQRCode = false;
	for (int a = 0; a < validFinderPatterns.size() - 2 && !isQRCode; a++) {
		for (int b = a + 1; b < validFinderPatterns.size() - 1 && !isQRCode; b++) {
			for (int c = b + 1; c < validFinderPatterns.size() && !isQRCode; c++) {
				DEBUG("Examining combination (%d, %d, %d) ...\n", a + 1, b + 1, c + 1);
				QRCode code;
				...
				DEBUG("Finding clockwise pattern order...\n");
				findClockwiseOrder(code);

				DEBUG("Finding top left corner pattern...\n");
				findTopLeftPattern(code);

				DEBUG("Finding merged lines...\n");
				if (!findMergedLines(code)) {
					DEBUG("Invalid merged lines. Combination is not a QRCode.\n");
					continue;
				}

				DEBUG("Finding corners...\n");
				findCorners(code);

				DEBUG("Finding perspective transform...\n");
				try
				{
				...
				{
					DEBUG("Invalid perspective transform. Combination is not a QRCode.\n");
					continue;
				}

				DEBUG("Finding number of modules...\n");
				if (!findNumberOfModules(code))
				{
					DEBUG("Invalid module values. Combination is not a QRCode.\n");
					continue;
				}

				DEBUG("Finding resized image...\n");
				normalize(code);
				...
				if (verifyQRCode(code)) {
					if (code.verifyPercentage < 85)
					{
						...
						DEBUG("Final Percentage: %f\n", code.verifyPercentage);
					}
				...
				else
				{
					...
					{
						DEBUG("Value below 65%%. Combination is not a QRCode.\n");
					}
				}
			}
		}
...
void CodeFinder::findCorners(QRCode &code) {
	code.corners = Mat(4, 4, traits::Type<Point2f>::value);
...
void CodeFinder::findPerspectiveTransform(QRCode &code)
{
	...
	for (Point2f p : sourceQuad) {
		if (p.x < 0 || p.y < 0) {
			DEBUG("The Points for perspectiveTransform have negative values.\n");
			throw exception();
		}
	}
	...
	for (int i = 0; i < 3; i++) {
		for (int j = i + 1; j < 4; j++) {
			if (sourceQuad[i] == sourceQuad[j]) {
				DEBUG("At least two Points for perspectiveTransform are equal.\n");
				throw exception();
			}
		}
	}
...
bool CodeFinder::findNumberOfModules(QRCode& code)
{
	...
	DEBUG("Grid Step Size (x,y): (%f,%f)\n", code.gridStepSize.x, code.gridStepSize.y);
	DEBUG("Approximated cell counts for X: %f and Y: %f\n", cellsX, cellsY);
	...
	if (version <= 0 || version > 40) {
		DEBUG("The versionnumber %d is invalid.\n", version);
		return false;
	}
...
bool CodeFinder::verifyQRCode(QRCode &code) {
	...
	DEBUG("Verified QRCode. Percentage is %f\n", code.verifyPercentage);
	if (code.verifyPercentage > 65)
	{
		return true;
	}
